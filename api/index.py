...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if bucket[0] >= 1:
        ...
        # Optimization: Read path directly from ASGI scope to avoid `URL` instantiation overhead
        req_path = request.scope.get("path", "") if hasattr(request, "scope") else ""
        logger.warning(f"Rate limit exceeded for IP: {client_ip} on path: {req_path}")
