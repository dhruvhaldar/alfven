...
## 2026-11-20 - Avoid Request.headers Instantiation Overhead
**Learning:** In FastAPI and Starlette middlewares, accessing `request.headers` (e.g. `request.headers.get("x-vercel-forwarded-for")`) instantiates a `starlette.datastructures.Headers` object. This instantiation is a slow process because it iterates over the entire `request.scope["headers"]` tuple list and parses strings every single time the property is accessed. In frequently executed middlewares like rate limiters and upload size limits, this introduces significant per-request overhead.
**Action:** When extracting a small number of specific headers in performance-critical paths (like middleware), extract the headers directly from the ASGI scope by iterating over `request.scope.get("headers", [])` and matching the byte strings (e.g. `k == b"content-length"`). This bypasses the dictionary instantiation overhead and yields measurable speedups.

## 2026-11-21 - JSONResponse serialization optimization
**Learning:** In FastAPI, returning a standard Python dictionary `{"altitude": [0.0, ...], "density": [1.0, ...]}` for endpoints that generate large payloads containing lists of primitives (like floats or integers) causes a severe performance bottleneck. FastAPI intercepts the dictionary and runs it through `jsonable_encoder`, a recursive utility that guarantees safety but adds significant overhead to large lists. My benchmarks showed returning a dictionary took ~7.4s vs ~2.9s when bypassing it.
**Action:** When an endpoint returns a large dictionary of natively JSON-serializable types (like long arrays of floats), explicitly wrap the return value in a `JSONResponse(content=result)`. This skips the recursive overhead of `jsonable_encoder` and drastically improves serialization speed. Ensure that unit tests calling the handler function directly are updated to unpack `json.loads(response.body)`.
