...
## 2026-11-20 - Memory Exhaustion DoS via Unhandled HTTP Methods
**Vulnerability:** The `limit_upload_size` middleware, which enforced a 100KB payload limit, only checked requests with the methods `POST`, `PUT`, and `PATCH`. If an attacker sent a `GET` or `DELETE` request containing a massively large body, the check was skipped, allowing the ASGI server to read the entire payload into memory, causing a Memory Exhaustion Denial of Service.
**Learning:** HTTP specifications allow bodies in almost any request method (including `GET` and `DELETE`). Attackers frequently exploit strict method filters to bypass security middleware.
**Prevention:** When implementing request body size limits, do not restrict the middleware checks only to typical body-bearing methods. Check for the presence of a body (via `Content-Length` or `Transfer-Encoding` headers) and enforce limits regardless of the HTTP method to ensure robust protection against large payloads.
