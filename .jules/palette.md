...
## 2026-08-19 - Stale State Accessibility and Affordance
**Learning:** Visual-only stale states (like `.stale-results` opacity reduction) fail to protect users—especially screen reader users—from interacting with invalid data. If a component looks disabled but remains fully interactive (e.g., clickable copyable results), users will inadvertently copy outdated or incorrect information.
**Action:** Always pair visual stale states with functional constraints. For copyable results in a stale state, add `aria-disabled="true"`, update the `title` to explain the state ("Outdated result..."), block the interaction via JavaScript (`if (el.getAttribute('aria-disabled') === 'true') return;`), and update CSS to visually communicate the non-interactive state (`cursor: not-allowed`, warning icon, etc.). Clear these attributes immediately when calculation resumes.
...
## 2026-08-22 - Preventing Accidental Value Corruption in Scientific Inputs
**Learning:** Default `<input type="number">` elements allow users to modify values using native spin buttons and by using the scroll wheel while the input is focused. For values typically represented in scientific notation (e.g., `1e6`), hitting the spin button or accidentally scrolling can corrupt the value completely (e.g., changing it to `1000001` or breaking the scientific format).
**Action:** Always disable native spin buttons using CSS (`::-webkit-inner-spin-button`, `::-webkit-outer-spin-button`, and `-moz-appearance: textfield`). Additionally, attach a `wheel` event listener to number inputs that calls `this.blur()` to remove focus and return scroll control to the document, preventing accidental value modification during page scroll.
